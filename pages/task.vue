<template>
  ...
  try {
    ...
    // ✅ 2. เรียก backend /tasks พร้อมส่ง Bearer Token
    const r = await fetch("http://localhost:8080/tasks", {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });

    // ✅ 3. ถ้า API error → แสดง debug + status
    if (!r.ok) {
      console.group("🧩 API /tasks Debug Info");
      ...
      // ✅ แสดง Alert และโยน Error พร้อม Status ชัดเจน
      alert(`❌ ${r.status} ${r.statusText}\n${err.detail || err.message || "Load failed"}`);
      throw new Error(`❌ ${r.status} ${r.statusText} - ${err.detail || err.message || "Load failed"}`);
    }

    // ✅ 4. ดึงข้อมูลจริงจาก Firestore
    const data = await r.json();
    const firebaseTasks = data.tasks || [];

    // ✅ 5. แปลงข้อมูลให้เข้ากับ mock structure เดิม
    tasks.value = firebaseTasks.map((t: any, idx: number) => ({
      id: idx + 1,
    ...
    // ✅ 6. แสดง error และ mock fallback
    console.error("❌ LoadTasks Fatal:", e);
    const msg = e?.message || String(e);
    pageError.value = msg.includes("❌") ? msg : `Load failed: ${msg}`;

    tasks.value = [
      {
        id: 1001,
        name: "Mock Task",
        description: "Fallback mock data (API unreachable)",
        time_specified: true,
        date: new Date().toISOString().slice(0, 10),
        time: "15:00:00",
        done: false,
